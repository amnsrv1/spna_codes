...
f= open("modes_dmn_scn_"+psrname+".txt","w+")
...
dm_modes =  #obtained in modes_dmn.py script

modes = [2,5,8,12]
sc_modes = np.floor([i *Tspan_years for i in modes])
sc_modes = sc_modes.astype(int)
print(sc_modes)

dmn = dm_noise_block(Tspan=Tspan, components=dm_modes)

        
for i in sc_modes:
	scn = chromatic_noise_block(gp_kernel='diag',idx=4,Tspan=Tspan, name='scat', components=i)
	...
	print(" For dmn_scn model, for sc modes as",i,",the log evidence is = {} ± {}".format(Dres.logz[-1],Dres.logzerr[-1]))
	
	print(" For dmn_scn model, for sc modes as",i,",the log evidence is = {} ± {}".format(Dres.logz[-1],Dres.logzerr[-1]),file=f)
